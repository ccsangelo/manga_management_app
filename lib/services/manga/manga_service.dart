...
class MangaService {
  ...
  static Future<void> init() async {
    ...
    // Schedule on event queue (not microtask queue) so first frame can render
    Future.delayed(Duration.zero, _removeExpiredEntries);
  }
  ...
  Future<Either<String, List<Manga>>> getPopular({bool nsfwEnabled = false}) async {
    final cacheKey = 'popular_now_$nsfwEnabled';
    final cached = _getCachedResult(cacheKey);
    ...
    try {
      ...
      var results = items
          .map((item) => Manga.fromJson(item as Map<String, dynamic>))
          .toList();
      if (!nsfwEnabled) results = results.where((m) => !_isNsfw(m)).toList();
  ...
  Future<Either<String, List<Manga>>> getLatestUpdates({bool nsfwEnabled = false}) async {
    final cacheKey = 'latest_updates_$nsfwEnabled';
    final cached = _getCachedResult(cacheKey);
    if (cached != null) return Right(cached.results);
  ...
  Future<Either<String, List<Manga>>> getRecommended(List<String> genres, {bool nsfwEnabled = false}) async {
    if (genres.isEmpty) return const Right([]);

    final keywords = genres.take(3).join(', ');
    final cacheKey = 'recommended_${genres.take(3).join('_')}_$nsfwEnabled';
    final cached = _getCachedResult(cacheKey);
    if (cached != null) return Right(cached.results);

    final result = await searchManga(keywords, nsfwEnabled: nsfwEnabled);
  ...
  Future<Either<String, List<Manga>>> getRandomBatch({bool nsfwEnabled = false}) async {
    try {
  ...
  Future<Either<String, MangaSearchResult>> getLatestUpdatesPaginated({int page = 1, bool nsfwEnabled = false}) async {
    final cacheKey = 'latest_updates_page_${page}_$nsfwEnabled';
    final cached = _getCachedResult(cacheKey);
    if (cached != null) return Right(cached);
  ...
  Future<Either<String, MangaSearchResult>> getTopMangaPaginated({
    ...
    final cacheKey = 'top_manga_${type ?? 'all'}_${filter ?? 'none'}_${page}_$nsfwEnabled';
    final cached = _getCachedResult(cacheKey);
    if (cached != null) return Right(cached);
